...
| `0x00000B90` | 2-3 | A/C Panel Line Voltage (Heartbeat) | `uint16` BE | x1 V | confirmed 1 Hz continuous heartbeat |
| `0x02140898` | 0 | MasterVolt 2000W Inverter Ramp | `uint8 enum` | `03=ON/Ramp-up, 02=OFF/Ramp-down` | confirmed; published as native Inverter service |
| `0x02141808` | 0-7 | Multibloc V8 Relay Active Status | `bitfield` | 15 channels | confirmed; published as native Victron switch service |
| `0x02140808` | 0-7 | Multibloc V8 Relay Mode / Key Feedback | `bitfield` | 15 channels | confirmed; bidirectional switch control |
...
## Smart Starter Battery Low-Voltage Alarm Policy

The bridge implements a smart low-voltage alarm evaluation for the generator and engine starter batteries:

* **Warning Threshold**: `< 12.2 V` sustained continuously for **15.0 seconds** (`/Alarms/LowVoltage = 1`).
* **Critical Alarm Threshold**: `< 11.8 V` sustained continuously for **15.0 seconds** (`/Alarms/LowVoltage = 2`).
* **Transient Cranking Rejection**: Voltage dips lasting `< 15.0 seconds` (e.g. 10.5V during engine/generator starter motor engagement) are ignored and do not trigger alarms.
* **Recovery Hysteresis**: Voltage must rise above `12.5 V` (threshold + 0.3V) to clear warnings/alarms, preventing alarm flapping.
...
A live test showed that 0 Hz (`STOPPED`) can precede `02440B88#00` (`OFF_IDLE`) by roughly a minute. A START sent during that settling interval was ignored by the Scheiber controller; a later START from `OFF_IDLE` succeeded.
...
The only production transmit ID in bridge v5.9.0 is `0x02460B88` (generator START/STOP) and `0x02161808` / `0x02140808` (switch momentary actuation). No automatic CAN retries are sent.
